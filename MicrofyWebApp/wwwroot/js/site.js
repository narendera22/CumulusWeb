...
function DownloadFile(url, phase, subphase, flag) {
    ...
    $.ajax({
        ...
        url: "/Repository/DownloadFile",
        data: data,
        success: function (r) {
            var resJson = JSON.parse(r);
            if (!resJson.isFileExists) {
                $.alert({
                    title: 'Error!!',
                    content: 'The file you are trying to download is no longer available.',
                    type: 'red'
                });
                return;
            }
            var bytes = Base64ToBytes(resJson.content);
